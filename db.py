...
async def get_user_stats(user_id):
    conn = await connect_db()
    user = await conn.fetchrow("SELECT usage_count, last_active FROM users WHERE user_id = $1", user_id)
    await conn.close()
    return user
...
async def get_daily_users():
    conn = await connect_db()
    daily_users = await conn.fetchval("SELECT COUNT(*) FROM users WHERE last_active::DATE = CURRENT_DATE;")
    await conn.close()
    return daily_users
